...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${cityName} Business Services | Visa, Company Setup & Digital Systems | VistaDocs`,
    description: `${location.description} VistaDocs provides complete business, visa, and digital services in ${cityName}. Contact us for a free consultation.`,
    keywords: [`${cityName.toLowerCase()} visa services`, `company formation ${cityName.toLowerCase()}`, `${cityName.toLowerCase()} business setup`, `${cityName.toLowerCase()} pro services`, `digital services ${cityName.toLowerCase()}`],
    alternates: {
      canonical: `/locations/${location.slug}`,
    },
    openGraph: {
      title: `${cityName} Business Services | VistaDocs`,
      description: `Expert business, visa, and digital services in ${cityName}, UAE`,
      url: `https://vistadocscenter.com/locations/${location.slug}`,
      type: 'website',
    },
  };
}
...
export default async function LocationPage({ params }: PageProps) {
  ...
  // LocalBusiness schema for location-specific SEO
  const localBusinessSchema = {
    '@context': 'https://schema.org',
    '@type': ['LocalBusiness', 'ProfessionalService'],
    name: `VistaDocs Center - ${cityName}`,
    ...
    priceRange: 'AED 1500 - AED 50000',
    serviceType: ['Visa Services', 'Company Formation', 'PRO Services', 'Digital Systems'],
  };
