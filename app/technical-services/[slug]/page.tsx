...
export async function generateStaticParams() {
  return Object.keys(technicalServicesData).map((slug) => ({
    slug: slug,
  }));
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const service = technicalServicesData[slug];
...
export default async function TechnicalServicePage({ params }: PageProps) {
  const { slug } = await params;
  const service = technicalServicesData[slug];
