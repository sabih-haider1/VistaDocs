...
export default function ServicePageClient({ service, category }: ServicePageClientProps) {
  ...
  return (
    <>
      <HeroSection title={service.seo.h1} description={service.seo.metaDescription} ctaText="Book Consultation" ctaHref="/contact" />

      <section className="section">
        <div className="container-xl">
          <div className="max-w-4xl mx-auto">
            {service.content.sections.map((section, index) => (
              <motion.div key={section.id} initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.06 }} className="mb-8 last:mb-0">
                <div className="card">
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <h2 className="text-2xl font-semibold text-[var(--text-900)] mt-1">{section.heading}</h2>
                  </div>
          ...
          <div className="container-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
              {service.linking.relatedServices.slice(0, 3).map((relatedSlug) => (
                <motion.a key={relatedSlug} href={`/${category}-services/${relatedSlug}`} whileHover={{ y: -4 }} className="block card hover:shadow-xl transition-shadow">
                  <h3 className="font-semibold text-[var(--text-900)] mb-2">{relatedSlug.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}</h3>
                  <p className="text-sm text-[var(--muted-text)]">Learn more about this service →</p>
                </motion.a>
              ))}
            </div>
          </div>
      ...
      <CTABlock title="Ready to Get Started?" description="Contact us today for a free consultation and let us help you with your requirements." buttonText="Request a Quote" buttonHref="/contact" />
    </>
  );
}
